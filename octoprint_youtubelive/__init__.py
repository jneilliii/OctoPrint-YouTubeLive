	...
	def on_api_command(self, command, data):
		...
		if command == 'startStream':
			...
			if not self.container:
				try:
					self.container = self.client.containers.run("octoprint/youtubelive:latest",command="pbea-b3pr-8513-40mh",detach=True,privileged=True,name="YouTubeLive",auto_remove=True)
					self._plugin_manager.send_plugin_message(self._identifier, dict(status=True,streaming=True))
				except Exception, e:
